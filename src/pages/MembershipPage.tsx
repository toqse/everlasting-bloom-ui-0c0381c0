...
const faqs: { question: string; answer: string | string[] }[] = [
  {
    question:
      "Why is Aiswarya Matrimonial better compared to other matrimonial websites?",
    answer:
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setPlans(WEBSITE_PLANS_FALLBACK);
          setFallbackNotice(getDisplayErrorMessage(e));
        }
      } finally {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section
        ...
        style={{ backgroundColor: "var(--membership-section-bg)" }}
      >
              ...
              plans.map((plan, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-[0_4px_12px_-2px_rgba(0,0,0,0.15)] ${ICON_BGS[index % ICON_BGS.length]}`}
                      >
                      ...
                      <h3
                        ...
                        style={{ color: "var(--membership-title)" }}
                      >
                        {plan.name}
                      </h3>
                      <p
                        className="text-xs mb-3"
                        style={{ color: "var(--membership-desc)" }}
                      >
                        {plan.description || "Membership plan"}
                      </p>

                      <div className="flex items-baseline gap-1 mb-3">
                        <span
                          className="font-serif text-3xl font-bold"
                          style={{ color: "var(--membership-price)" }}
                        >
                          {formatCurrency(plan.price)}
                        </span>
                        <span
                          className="text-sm"
                          style={{ color: "var(--membership-period)" }}
                        >
                      ...
                      {typeof plan.total_price?.female === "number" && (
                        <p
                          className="text-sm font-medium mb-1 underline underline-offset-2"
                          style={{ color: "var(--membership-feature)" }}
                        >
                          Total starts at{" "}
                          {formatCurrency(plan.total_price.female)}
                        </p>
                      )}
                      ...
                      {typeof plan.service_charge?.female === "number" && (
                        ...
                        >
                          Service charge{" "}
                          {formatCurrency(plan.service_charge.female)} onwards
                        </p>
                      )}
                          ...
                          <li key={i} className="flex flex-col">
                            <div className="flex items-start gap-2.5 py-3">
                              <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 bg-[#6A3266]/15">
                                <Check
                                  className="w-3 h-3"
                                  style={{ color: "var(--membership-check)" }}
                                />
                              </div>
                              <span
                            ...
                            {i < features.length - 1 && (
                              <div
                                ...
                                style={{
                                  borderColor: "var(--membership-divider)",
                                }}
                              />
                            )}
                          </li>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-8">
            {[
              ...
              <div
                key={index}
                className="flex items-center gap-4 bg-white/80 backdrop-blur-sm px-6 py-4 rounded-2xl shadow-soft hover-lift"
              >
            ...
            {faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-5 h-5 text-primary transition-transform flex-shrink-0 ${openFaq === index ? "rotate-180" : ""}`}
                  />
                </button>
                {openFaq === index && (
